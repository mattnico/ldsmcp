...
export const browseStructureTool = {
  definition: {
    ...
    description: "Browse the hierarchical structure of Gospel Library content",
    inputSchema: {
      type: "object",
      properties: {
        uri: {
  ...
  handler: async (args: BrowseStructureArgs) => {
    ...
    let results: StructureBrowseResult[] = [];
    ...
    try {
      ...
      results = gospelLibraryClient.parseDynamicContent(response);
      ...
      if (results.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: `No structural content found for "${uri}". This may be a leaf node or the content may not be available.`,
            },
          ],
        };
      }
          ...
          if (result.type === 'collection' || result.type === 'book') {
            try {
              ...
              if (!childResponse.error) {
                ...
                result.children = childResults.slice(0, 10);
              }
            } catch (error) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error browsing structure: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
...
function mapContentTypeToStructureType(contentType?: string): StructureBrowseResult['type'] {
  switch (contentType) {
    case 'general-conference': return 'session';
    case 'scripture': return 'chapter';
    case 'manual': return 'book';
    case 'come-follow-me': return 'book';
    case 'music': return 'collection';
    case 'magazine': return 'collection';
